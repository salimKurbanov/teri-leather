...
export default function BasketItem({product, setBasket}) {

    const [count, setCount] = useState(() => {
        const basket = JSON.parse(localStorage.getItem('basket'));
        const found = basket?.find(({ id }) => id === product.id);
    ...
    const minus = async () => {
        ...
        Store.sendMessage('Кол-во товара уменьшено')
    }
    ...
    const plus = async () => {
        ...
        Store.setListener('count_main', Basket.count())
        Store.setListener('total_price', Basket.totalPrice())
        Store.sendMessage('Кол-во товара увеличено')
    }
